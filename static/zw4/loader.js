...
// helper function for custom map loader
// gets a loot from the loot tier
function getLoot(name) {
	var sum = 0;
	for (var x of lootTables[name]) {
		sum += x[0];
	}
	...
	// let's say something bugged out, we were not able to find something to drop, drop the 0th item in the loot table
	return lootTables[name][0][1];
}
			...
			for (const geom of data.geometries) {
				...
				if (geom.material.startsWith("hbmat_")) {
					...
					res.items.push([toPush[0][0], toPush[0][1], toPush[0][2],
						...getLoot( geom.material.split(".")[0] )  ]);
				} else if (geom.material.startsWith("recorder_")) {
					res.recorders.push([toPush, geom.material]);
				}
				else if (geom.material.startsWith("crate_")) {
					geom.material = geom.material.replace("heal_", "");
				}
				else {
