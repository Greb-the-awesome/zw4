...
class ObjectInHandRenderer {
	// yeah so it renders an object that's in your hand like a gun, a consumable, etc
	// if you want to render something, first tell us that you want to render it by calling addToBuffer

	// addresses is in the format:
	// 		name: renderBuffer id
	static addresses = {};
	static shaders = {}; // [name, shaderName]
	static addToBuffer(name, data, shader) {
		addresses[name] = createRenderBuffer(shader);
		var rbData = getRBdata(addresses[name], name);
		for (var prop of rbData) {
			rbData[prop] = rbData[prop].concat(data[prop]);
		}
	}
	static renderObject(name, mvm) {
		// draw`from the rb
		// WARNING: you gotta switch back attribute bindings and shaders yourself
		useShader(shaders[name]);
		var oldMVM = new Float32Array(modelViewMatrix);
		modelViewMatrix = mvm;
		useRenderBuffer(addresses[name], shaders[name]);
		var prop = Object.keys(buffers_d[name].buffer)[0];
		gl.drawArrays(gl.TRIANGLES, 0, getRBdata(addresses[name], shaders[name])[buffers_d[name].data[prop]].length /
			buffers_d[name].buffer[prop][0]);
	}
}
...
class Item extends PhysicsObject {
	...
	constructor(x, y, z, thing, type, add = true) {
		super(x, y, z, 0.49, 0.49, 0.49, false, false, true);
		this.type = type;
		this.thing = thing;
		this.ignores = new Set(["Player"]);
		this.t = 0; // used for the rotation effect, in seconds
		if (add) items.push(this);
		// hb2 is used to give it a trigger collider to detect the player
		this.hb2 = new PhysicsObject(x, y, z, 0.5, 0.5, 0.5, false, true, true);
		this.hb2.ignores = new Set(["Item"]); // prevent the two hitboxes from colliding with each other
		let it = this;
		this.hb2.onCollision = function(obj, n) {
			if (it.removed) {return;} // since items may not be removed instantly
			if (it.type == "gun") {
				if (obj.constructor.name == "Player") {
					renderInteractionPrompt("F - " + it.thing);
					if (divisDownKeys["KeyF"]) {
						var found = false;
						for (var i=0; i<4; i++) {
							if (player.inv[i].constructor.name == "NothingGun") {
								player.inv[i] = new Gun(it.thing);
								found = true;
								player.invSelect = i;
								break;
							}
						...
						if (!found) {
							...
							player.inv[player.invIndex] = new Gun(it.thing);
	...
	static renderAll() {
		for (var ib of items) {
			if (glMatrix.vec3.distance(ib.pos, player.pos) < levelSpecs[currentLevel.levelNum].simulationDistance || true) {
				ib.render();
			}
		}
	}
...
class Consumable {
	constructor(name) {
		this.name = name;
		this.specs = consumableSpecs[name];
	}
...
class Player extends PhysicsObject {
	...
	genPathfindingMesh() {
		PathfinderInterface.sendPhysicsObjects();
		PathfinderInterface.genGrid(0.5, 0.7, 2, 3, [this.pos[0], this.pos[1], this.pos[2]]);
	}
	update(dt) {
		// update selected
...
class Level {
	...
	load() {
		...
		IHP.simulationDistance_kin = levelSpecs[this.levelNum].physicsSimulationDistanceR;
		IHP.simulationDistance_notKin = levelSpecs[this.levelNum].physicsSimulationDistanceG;
